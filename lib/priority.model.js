  ...
  async.parallel({
    ...
    serviceGroup: function checkServiceGroupDependency(next) {
      ...
      if (ServiceGroup) {
        ...
        ServiceGroup
          .count({ priority: this._id }, function (error, count) {

            //warning can not delete
            if (count && count > 0) {
              const errorMessage =
                `Fail to Delete. ${count} service groups depend on it`;
...
PrioritySchema.statics.findDefault = function (done) {
  ...
  Priority.findOne().sort({ weight: 'asc' }, done); //TODO cache in memory
...
module.exports = mongoose.model(PRIORITY_MODEL_NAME, PrioritySchema);
