...
describe('Priority', function () {
  ...
  describe('Rest API', function () {
    ...
    it('should handle HTTP GET on /priorities/id:', function (done) {
      ...
      request(app)
        .get(`/v1.0.0/priorities/${priority._id}`)
        .set('Accept', 'application/json')
        .expect(200)
        .end(function (error, response) {
          expect(error).to.not.exist;
    ...
    it('should handle HTTP PATCH on /priorities/id:', function (done) {
      ...
      request(app)
        .patch(`/v1.0.0/priorities/${priority._id}`)
        .set('Accept', 'application/json')
        .set('Content-Type', 'application/json')
        .send(patch)
        .expect(200)
        .end(function (error, response) {
          ...
          expect(patched.color).to.be.equal(priority.color);
    ...
    it('should handle HTTP PUT on /priorities/id:', function (done) {
      ...
      request(app)
        .put(`/v1.0.0/priorities/${priority._id}`)
        .set('Accept', 'application/json')
        .set('Content-Type', 'application/json')
        .send(put)
        .expect(200)
        .end(function (error, response) {
          ...
          expect(puted.color).to.be.equal(priority.color);
    ...
    it('should handle HTTP DELETE on /priorities/:id', function (done) {
      ...
      request(app)
        .delete(`/v1.0.0/priorities/${priority._id}`)
        .set('Accept', 'application/json')
        .expect(200)
        .end(function (error, response) {
          expect(error).to.not.exist;
          ...
          expect(deleted.color).to.be.equal(priority.color);
